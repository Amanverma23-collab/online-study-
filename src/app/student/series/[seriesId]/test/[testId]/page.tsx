...
import { User, Info, BookOpen, ChevronLeft, ChevronRight, Check } from "lucide-react";
...
export default function SeriesTestInterface({ params }: { params: { seriesId: string; testId: string } }) {
  ...
  const initializeAttempt = async (sId: string) => {
    try {
      // 1. Start or resume series attempt
      const startRes = await fetch("/api/series-attempts/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ studentId: sId, seriesTestId: testId })
      });
  ...
  const loadSectionDetails = async (attId: string) => {
    try {
      ...
      const sectionRes = await fetch(`/api/series-attempts/${attId}/current-section`);
  ...
  const triggerAutoSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/series-attempts/${attemptId}/submit-section`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const saveAnswerToDb = async (qId: string, selected: string | null, isMarked: boolean, isVisited: boolean) => {
    try {
      await fetch(`/api/series-attempts/${attemptId}/save-answer`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          questionId: qId,
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#0D0F12] text-[#EEF0E8] flex flex-col justify-between select-none font-body">
      {/* 1. TOP BAR */}
      <header className="bg-[#1C2415] border-b border-[#2E3B1E] px-6 py-3 flex justify-between items-center h-16 flex-shrink-0">
        <div className="flex items-center gap-3">
          <span className="label-badge text-[#8B9E6A]">Sections:</span>
            ...
            {allSections.map((sec) => {
              ...
              return (
                <button
                  key={sec.id}
                  disabled
                  className={`label-badge px-3 py-1.5 rounded-sm uppercase flex items-center gap-1.5 transition text-[10.5px] font-bold ${secClass}`}
                >
                  {sec.subject}
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <span className="text-xs text-[#8B9E6A] font-display font-bold uppercase tracking-wider">Time Left:</span>
            <span className={`font-mono text-xl font-bold border-none ${
          ...
          <div className="flex items-center gap-2 text-xs text-[#8B9E6A] font-display font-bold uppercase tracking-wider border-l border-[#2E3B1E] pl-4">
            <span>Language: </span>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col justify-between overflow-y-auto bg-[#0D0F12]">
          <div className="p-8 max-w-4xl w-full mx-auto">
            ...
            <div className="bg-[#1C2415] p-6 rounded border border-[#2E3B1E] shadow-sm mb-6">
              ...
              {(["A", "B", "C", "D"] as const).map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-start text-left gap-4 p-4 rounded border transition duration-150 ${
                      isSelected
          ...
          <footer className="sticky bottom-0 bg-[#1C2415] border-t border-[#2E3B1E] px-6 py-4 flex justify-between items-center z-10">
            <div className="flex gap-3">
              {currentState.isMarked ? (
                <button
                  onClick={handleUnmarkReview}
                  className="btn-secondary text-xs px-4 py-2.5 bg-[#6B4E8A]/20 border-[#6B4E8A] text-[#EEF0E8] hover:bg-[#6B4E8A]/45"
                >
                  Unmark Review
                </button>
              ) : (
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="p-2.5 border border-[#2E3B1E] bg-[#0D0F12] rounded hover:bg-[#1C2415] disabled:opacity-30 disabled:hover:bg-transparent transition"
              >
                <ChevronLeft className="w-5 h-5 text-[#8B9E6A]" />
              </button>
              <button
                onClick={() => navigateToQuestion(currentIndex + 1)}
                ...
                className="p-2.5 border border-[#2E3B1E] bg-[#0D0F12] rounded hover:bg-[#1C2415] disabled:opacity-30 disabled:hover:bg-transparent transition"
              >
                <ChevronRight className="w-5 h-5 text-[#8B9E6A]" />
              </button>
        ...
        <aside className="w-80 border-l border-[#2E3B1E] bg-[#1C2415] flex flex-col justify-between hidden md:flex flex-shrink-0">
          <div className="overflow-y-auto flex-1">
            {/* Student info */}
            <div className="p-4 border-b border-[#2E3B1E] flex items-center gap-3 bg-[#0D0F12]/60">
