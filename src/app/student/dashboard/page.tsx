...
import { Star, LogOut, Radio, Clock, Award, FileText, CheckCircle, ChevronRight, Package, IndianRupee, Play, Lock } from "lucide-react";
...
export default function StudentDashboardPage() {
  ...
  // Filter available series that the student is NOT enrolled in and sort matching batch first
    ...
    <div className="h-screen overflow-hidden bg-[#F5F3EC] flex flex-col text-[#0D0F12] font-body">
      ...
      <header className="sticky top-0 z-50 bg-[#0D0F12] py-4 px-6 md:px-12 text-[#EEF0E8] shadow flex justify-between items-center border-b border-[#2E3B1E] flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-[#C9A84C] rounded flex items-center justify-center text-[#0D0F12]">
        ...
        <div className="flex items-center gap-6">
          <span className="text-xs font-display font-bold uppercase tracking-wider text-[#EEF0E8] hidden md:inline">
            Candidate: <span className="text-[#C9A84C] font-semibold">{studentName}</span> 
            <span className="ml-2 bg-[#2E3B1E] text-[#8B9E6A] px-2 py-0.5 rounded text-[10px] font-mono">{studentBatch}</span>
          </span>
          <button onClick={handleLogout} className="flex items-center gap-2 text-xs font-display font-bold uppercase tracking-wider text-[#8B9E6A] hover:text-[#EEF0E8] transition cursor-pointer">
      ...
      {loading ? (
        ...
        <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-8 space-y-10 lg:space-y-6 overflow-y-auto lg:overflow-hidden lg:h-[calc(100vh-72px)] lg:flex lg:flex-col no-scrollbar">
          ...
          {nonEnrolledSeries.length > 0 && (
            <section className="flex-shrink-0">
              <div className="flex items-center gap-2.5 border-b border-[#DDD8CC] pb-3 mb-6">
                ...
                <h2 className="font-display font-bold text-xl uppercase tracking-wider text-[#0D0F12]">Paid Test Series</h2>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:flex-1 lg:min-h-0">
            {/* Left: Free Live Tests */}
            <div className="lg:col-span-2 flex flex-col lg:h-full lg:min-h-0 space-y-4">
              <div className="flex items-center gap-2.5 border-b border-[#DDD8CC] pb-3 flex-shrink-0">
                <Radio className="w-5 h-5 text-[#C9A84C]" />
                <h2 className="font-display font-bold text-xl uppercase tracking-wider text-[#0D0F12]">Live Briefings & Exams</h2>
              </div>
                      ...
                      return (
                        <div key={test.id} className="bg-white rounded-[6px] border border-[#DDD8CC] p-6 shadow-sm hover:shadow-md transition duration-150 flex flex-col justify-between">
                          <div>
                            ...
                            <h3 className="font-display font-bold text-md text-[#0D0F12] mb-2">{test.title}</h3>
            ...
            <div className="flex flex-col lg:h-full lg:min-h-0 space-y-4">
              <div className="flex items-center gap-2.5 border-b border-[#DDD8CC] pb-3 flex-shrink-0">
                ...
                <h2 className="font-display font-bold text-xl uppercase tracking-wider text-[#0D0F12]">My Attempts</h2>
              </div>
                    ...
                    {attempts.map((att) => (
                      <div key={att.id} className="py-4 first:pt-0 last:pb-0 flex justify-between items-center gap-4 hover:bg-gray-50/50 px-2 rounded transition">
                        <div className="space-y-1">
                          <h4 className="font-display font-bold text-sm text-[#0D0F12] leading-tight line-clamp-1 uppercase">{att.test.title}</h4>
          ...
          {enrolledSeries.length > 0 && (
            <section className="flex-shrink-0">
              <div className="flex items-center gap-2.5 border-b border-[#DDD8CC] pb-3 mb-6">
                ...
                <h2 className="font-display font-bold text-xl uppercase tracking-wider text-[#0D0F12]">My Enrolled Test Series</h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
