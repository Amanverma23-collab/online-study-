...
export async function POST(
  ...
  try {
    ...
    // Check if section result already exists
    const existingSectionResult = await db.seriesSectionResult.findFirst({
    ...
    if (isLastSection) {
      ...
      const finalAttempt = await db.seriesAttempt.update({
        where: { id: attemptId },
        data: {
      ...
      // Recalculate ranks and percentiles for this series test
    ...
    console.error("Submit Series Section Error:", error);
