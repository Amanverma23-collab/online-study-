...
export async function GET(
  ...
  try {
    ...
    if (attempt.submittedAt !== null) {
      return NextResponse.json({ finished: true, redirectTo: `/student/series/${attempt.seriesTest.seriesId}/result/${attemptId}` });
    }
    ...
    while (!hasCheckedExpiry) {
      ...
      if (elapsedSeconds >= totalSeconds) {
        // Section has expired!
        const sectionQuestions = await db.seriesQuestion.findMany({
          where: { sectionId: activeSection.id }
        });
        const sectionAnswers = await db.seriesAnswer.findMany({
          where: {
            attemptId,
    ...
    const answers = await db.seriesAnswer.findMany({
      where: {
        attemptId,
    ...
    const sectionResults = await db.seriesSectionResult.findMany({
      where: { attemptId }
    });
    ...
    console.error("GET Current Section Series Error:", error);
