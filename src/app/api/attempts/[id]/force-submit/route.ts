...
export async function POST(
  ...
  try {
    ...
    const existingSR = await db.sectionResult.findFirst({
      where: { attemptId, sectionId }
    });

    if (existingSR) {
      await db.sectionResult.update({
    ...
    const completedSectionResults = await db.sectionResult.findMany({
      where: { attemptId }
    });
    ...
    // 6. Recalculate ranks and percentiles for this test
